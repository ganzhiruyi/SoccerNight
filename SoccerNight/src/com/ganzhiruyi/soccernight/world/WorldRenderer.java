...
public class WorldRenderer {
	...
	public void render(){
		...
		if(world.getState() == World.WORLD_STATE_GAME_OVER) return;
		cam.update();
	...
	private void renderBackground() {
		...
		batch.begin();
		batch.draw(Assets.level_1_bg, 0, 0, Config.SCREEN_WIDTH, Config.SCREEN_HEIGHT);
		batch.end();
	}
	...
	private void renderZombies(){
		...
		for(Zombie z : zombies){
			...
			switch(z.getState()){
			case IDLE:
				region = z.isRight ?  Assets.aniZombieIdleR : Assets.aniZombieIdleL;
				batch.draw(region, x, y, Zombie.ZOMBIE_WIDTH, Zombie.ZOMBIE_HEIGHT);
				break;
			case MOVING:
				float vx = z.velocity.x;
				if(vx < 0){
					region = Assets.aniZombieL.getKeyFrame(stateTime, Animation.ANIMATION_LOOPING);
					batch.draw(region, x, y, Zombie.ZOMBIE_WIDTH, Zombie.ZOMBIE_HEIGHT);
				}
				else if(vx > 0){
					region = Assets.aniZombieR.getKeyFrame(stateTime, Animation.ANIMATION_LOOPING);
					batch.draw(region, x, y, Zombie.ZOMBIE_WIDTH, Zombie.ZOMBIE_HEIGHT);
				}
				break;
			default:
